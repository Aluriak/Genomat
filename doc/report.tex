...
\subsection{Mutation rate variation}
    ...
    \paragraph*{}
    As each gene mutates, there is no constant resistance to gene KO survivability ratio stabilization, as the telltale. 
    ...
    (as far as a natural population can be composed of only five genes).
    \paragraph*{}
    ...
    for a given population size and a given genes network, the same results will always be generated when using mutation rates low enough.
    \paragraph*{}
...
\subsection{Parents number variation}
    ...
    \end{figure}
    \paragraph*{}
    When the parents number is equals to the population size, its just simulate an only one big pool of genes instead of a lot of smaller ones. 
